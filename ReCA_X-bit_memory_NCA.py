...
class ReservoirMemorySingleExperiment:
    ...
    def set_up_evodynamics_v2(self):
        prob_std = self.ca_rule
        fargs_list = [( [0.39422172047670734, 0.09472197628905793,\
                   0.2394927250526252, 0.4084554505943707, 0.0,\
                   0.7302038703441202, 0.9150343715586952, 1.0, prob_std],)]

        # Training delay is equals to 5 because it is a 5-bit memory experiment
        # exp = experiment.Experiment(input_start=0, input_delay=self.reservoir_height, training_delay=5)
        exp = experiment.Experiment(input_start=0, input_delay=self.reservoir_height)
        g_ca = exp.add_group_cells(name="g_ca", amount=self.ca_width)
        neighbors, center_idx = ca.create_pattern_neighbors_ca1d(3)
        g_ca_bin = g_ca.add_binary_state(state_name='g_ca_bin', init="zeros")
        g_ca_bin_conn = ca.create_conn_matrix_ca1d(
        ...
        exp.add_connection(
            ...
            connection.WeightedConnection(
                g_ca_bin,
                g_ca_bin,
                act.rule_binary_soc_sca_1d_width3_func,
                g_ca_bin_conn,
                fargs_list=fargs_list))
    ...
    def save_img(self):
        ...
        for i in range(0, 2 ** self.number_of_bits):
            ...
            arr = np.array(self.exp_history[((i + 1) * self.ca_height) - 1])
            ...
            # img = Image.fromarray(arr.resize(5*arr.shape[0],5*arr.shape[1]), Image.NEAREST)
...
def recordingExp(bits, r_total_width, d_period, ca_rule, runs, r, itr):
    filename = f'stoch {datetime.now().isoformat().replace(":", " ")}.txt'
...
# prob_std_list = np.concatenate((np.linspace(0,2,21), np.array([5,10,20,50,100,200,500,1000])))
prob_std_list = [0.0]#np.linspace(1.5,2.0,6)

# for ca_rule in range(0, 256):
for ca_rule in prob_std_list:
